    ...
    methods: {
      ...
      updatePassword(){
        ...
        this.$confirm('确认提交吗？', '提示', {}).then(() => {
          loginApi.update_password(this.params.condition.classId,this.updatePwd.oldPassword,this.updatePwd.newPassword2).then((res) => {
            if(res.success){
              this.$message({
      ...
      query: function (par) {
        ...
        //调用服务端的接口
        loginApi.class_list(this.params.page,this.params.size,this.params.condition).then((res) => {
          this.user.username = res.queryResult.list[0].className;
