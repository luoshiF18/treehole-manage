      ...
      <el-table-column label="操作" width="300">
        <template slot-scope="page">
          <el-button
            ...
            @click="edit(page.row.teacherId)">编辑
          </el-button>
          <el-button
            size="small"type="text"
            @click="del(page.row.teacherId)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
   ...
   methods: {
       ...
       edit(teacherId){
          this.$router.push({
              path:'/informationmanage/page/teacher/edit/'+teacherId
          })
       },
       //删除老师信息
       del:function (teacherId) {
           this.$confirm('确认删除此页面吗?', '提示', {}).then(() => {
               trainApi.del_teacher(teacherId).then((res)=>{
                   if(res.success){
                       this.$message({
                           type: 'success',
                           message: '删除成功!'
