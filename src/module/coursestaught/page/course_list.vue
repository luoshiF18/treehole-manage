...
<script>
   ...
   methods: {
       ...
       query: function (par) {
         ...
         teacherApi.teacherCourseOfTeaching(this.params.page,this.params.size,this.params.condition).then((res) => {
               //将res结果数据赋值给数据模型对象
     ...
     upload(classCourseId){
       this.$router.push({
         path:'/coursestaught/page/upload/'+classCourseId
       })
     },
     //跳转到下载文件页面
     download(classCourseId){
       this.$router.push({
         path:'/coursestaught/page/download/'+classCourseId
       })
     },
       //查询课程类型
